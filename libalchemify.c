...
static int write_ints(FILE * f, int *p, int n, int per_line, int ext);
    ...
    *nInitial = 0;
    ...
    while (!feof(f)) {
        ...
        if (line[0] == 'A' && line[1] == 'T' && line[2] == 'O' && line[3] == 'M') {
            ...
            if (input == 1.0) {
                if (*nFinal >= MAX_GROUP_SIZE)
                    CLOSE_AND_DIE(f, "too many final atoms");
                final[(*nFinal)++] = natoms;
            }
            if (input == -1.0) {
                if (*nInitial >= MAX_GROUP_SIZE)
                    CLOSE_AND_DIE(f, "too many initial atoms");
...
/* Does the job                                        */
/************************************************************************************/
int process(FILE * in, FILE * out, int natoms, int *initial, int *final, int nInitial, int nFinal)
{
    ...
    int wrong = 0;
    int ext = 0;                /* are we looking at a CHARMM EXTENDED PSF? */

    if (!fgets(buf, 512, in))
        DIE("error or EOF while reading PSF header");
    ...
    /* read title */
    if (fscanf(in, "%i !NTITLE", &n) != 1)
        DIE("could not read number of title lines");
    if (skip_line(in))
        DIE("unexpected EOF");
    fprintf(out, ext ? "%10d !NTITLE\n" : "%8d !NTITLE\n", n);

    for (i = 0; i < n; i++) {
        if (!fgets(buf, 512, in))
            DIE("error or EOF while reading title");
        fputs(buf, out);
    }
    if (skip_line(in))
        DIE("unexpected EOF before atoms");
    fputc('\n', out);

    /* read atoms */
    if (fscanf(in, "%i !NATOM", &n) != 1)
        DIE("could not read number of atoms");
    if (n != natoms)
        DIE("incorrect number of atoms in PSF file");
    ...
    for (i = 0; i < n; i++) {
        if (!fgets(buf, 512, in))
            DIE("error or EOF while reading atoms");
        fputs(buf, out);
    }

    /* number of bonds */
    if (skip_line(in))
        DIE("unexpected EOF before BONDS");
    fputc('\n', out);           /* blank line */

    if (fscanf(in, "%i !NBOND", &n) != 1)
        DIE("could not read number of bonds");
    if (skip_line(in))
        DIE("unexpected EOF before BONDS");
    ...
    if (n) {
        ...
        /* READ */
        if (read_ints(in, p, 2 * n))
            FREE_AND_DIE(p, "reading BONDS");
        if (skip_line(in))
            FREE_AND_DIE(p, "unexpected EOF before ANGLES");
        ...
        fprintf(out, ext ? "%10i !NBOND: bonds\n" : "%8i !NBOND: bonds\n", n);
        for (i = 0; i < n; i++)
            write_ints(out, p + 2 * i, 2, 4, ext);

        write_ints(NULL, NULL, 0, 0, ext);      /* end section */
        free(p);
    } else {
        putc('\n', stdout);
        fprintf(out, ext ? "%10d !NBOND: bonds\n" : "%8d !NBOND: bonds\n", 0);
    }
    ...
    /* angles */
    if (fscanf(in, "%i !NTHETA", &n) != 1)
        DIE("could not read number of angles");
    if (skip_line(in))
        DIE("unexpected EOF before ANGLES");
    ...
    if (n) {
        ...
        /* READ */
        if (read_ints(in, p, 3 * n))
            FREE_AND_DIE(p, "reading ANGLES");
        if (skip_line(in))
            FREE_AND_DIE(p, "unexpected EOF before ANGLES");
        ...
        fprintf(out, ext ? "%10d !NTHETA: angles\n" : "%8d !NTHETA: angles\n", n - removed);
        for (i = 0; i < n; i++)
            if (!couples(p + 3 * i, 3, initial, final, nInitial, nFinal))
                write_ints(out, p + 3 * i, 3, 3, ext);
    ...
    /* dihedrals */
    if (fscanf(in, "%i !NPHI", &n) != 1)
        DIE("could not read number of dihedrals");
    if (skip_line(in))
        DIE("unexpected EOF before dih");
    ...
    if (n) {
        ...
        /* READ */
        if (read_ints(in, p, 4 * n))
            FREE_AND_DIE(p, "reading DIHEDRALS");
        if (skip_line(in))
            FREE_AND_DIE(p, "unexpected EOF before ANGLES");
        ...
        fprintf(out, ext ? "%10d !NPHI: dihedrals\n" : "%8d !NPHI: dihedrals\n", n - removed);
        for (i = 0; i < n; i++)
            if (!couples(p + 4 * i, 4, initial, final, nInitial, nFinal))
                write_ints(out, p + 4 * i, 4, 2, ext);
    ...
    /* impropers */
    if (fscanf(in, "%i !NIMPHI", &n) != 1)
        DIE("could not read number of impropers");
    if (skip_line(in))
        DIE("unexpected EOF before impropers");
    ...
    if (n) {
        ...
        /* READ */
        if (read_ints(in, p, 4 * n))
            FREE_AND_DIE(p, "reading IMPROPERS");
        if (skip_line(in))
            FREE_AND_DIE(p, "unexpected EOF before IMPR");
        ...
        fprintf(out, ext ? "%10d !NIMPHI: impropers\n" : "%8d !NIMPHI: impropers\n", n - removed);
        for (i = 0; i < n; i++)
            if (!couples(p + 4 * i, 4, initial, final, nInitial, nFinal))
                write_ints(out, p + 4 * i, 4, 2, ext);
    ...
    /* donors */
    if (fscanf(in, "%i !NDON", &n) != 1)
        DIE("could not read number of donors");
    if (skip_line(in))
        DIE("unexpected EOF");
    ...
    if (n) {
        for (i = 0; i < n; i++) {
            if (read_ints(in, tab, 2))
                DIE("reading DONORS");
            write_ints(out, tab, 2, 4, ext);
        }
        write_ints(NULL, NULL, 0, 0, ext);      /* end section */
        if (skip_line(in))
            DIE("unexpected EOF");
    ...
    /* acceptors */
    if (fscanf(in, "%i !NACC", &n) != 1)
        DIE("could not read number of acceptors");
    if (skip_line(in))
        DIE("unexpected EOF");
    ...
    /* exclusions */
    if (fscanf(in, "%i !NNB", &n) != 1)
        DIE("could not read number of exclusions");
    if (skip_line(in))
        DIE("unexpected EOF");
    ...
    for (i = 0; i < n; i++) {
        if (read_ints(in, tab, 1))
            DIE("reading exclusion lists");
    }
    /* read and forget the indices */
    for (i = 0; i < natoms; i++) {
        if (read_ints(in, tab, 1))
            DIE("reading exclusion list indices");
    }
    ...
    for (i = 0; i < nInitial; i++)
        for (j = 0; j < nFinal; j++)
            write_ints(out, final + j, 1, 8, ext);
...
/************************************************************************************/
int write_ints(FILE * f, int *p, int n, int per_line, int ext)
{
    ...
    for (i = 0; i < n; i++) {
        fprintf(f, ext ? "%10i" : "%8i", p[i]);
    }
    if (i)
        written++;              /* unless we wrote nothing */
